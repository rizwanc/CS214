...
/*
 * Structure for creating a Node type.  
 * Contains void* pointer, reference counter, pointer to next item
 */
struct node_T
{
   void* objContents;
...
/*
 * Creates a new node and returns a pointer to that node if it is a success
 * otherwise return null
 */
node *createNode(void* objContents, node* nextElement);

/*
 * Destroy and free memory of node
 * Method called by SLNextItem when the reference countet is equal to 1
 * It will deduct 1 from the reference counter of the next node if 
 * nextElement != null.
 */ 
